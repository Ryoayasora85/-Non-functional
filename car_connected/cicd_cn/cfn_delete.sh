...
function swichRoleCmd() {
  ...
  while read line
  do
    if [[ $line =~ "StackName" ]] && [[ $line =~ "monitoring-lambda" ]]; then
      aaaaa=${line//\,/}
      bbbbb=${aaaaa//\"/}
      ccccc=`echo $bbbbb|awk '{print $2}'`
      echo "aws cloudformation delete-stack --stack-name ${ccccc//\"/}"
      aws cloudformation delete-stack --stack-name ${ccccc//\"/}
    fi
  done <elb.txt
