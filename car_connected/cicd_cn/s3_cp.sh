...
function swichRoleCmd() {
  ...
  do
    if [[ $line =~ "cfn" ]]; then
      aaaaa=${line//\,/}
      bbbbb=${aaaaa//\"/}
      ccccc=`echo $bbbbb|awk '{print $3}'`
      echo "mv ./botopython/${ccccc}.json"
      mv ./botopython/${ccccc}.json ./monitor.json
      echo "aws s3 cp ./monitor.json s3://${ccccc}/monitor-config/"
      aws s3 cp ./monitor.json s3://${ccccc}/monitor-config/
      #aws s3 cp ./monitoring_common_lambda_src.zip s3://`echo $bbbbb|awk '{print $3}'`/CloudwatchAlarm/
      #echo "aws s3 rm s3://`echo $bbbbb|awk '{print $3}'`/CloudwatchAlarm/monitoring_common_lambda_src.zip"
    fi
  done <elb.txt
